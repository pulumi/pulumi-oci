// Code generated by the Pulumi Terraform Bridge (tfgen) Tool DO NOT EDIT.
// *** WARNING: Do not edit by hand unless you're certain you know what you are doing! ***

package disasterrecovery

import (
	"context"
	"reflect"

	"errors"
	"github.com/pulumi/pulumi-oci/sdk/go/oci/internal"
	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
)

// This resource provides the Dr Protection Group resource in Oracle Cloud Infrastructure Disaster Recovery service.
//
// Create a DR protection group.
//
// ## Create
//
// Create DR Protection Group resource with a default value of `disassociateTrigger` property, e.g.
//
// ```go
// package main
//
// import (
//
//	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
//
// )
//
//	func main() {
//		pulumi.Run(func(ctx *pulumi.Context) error {
//			return nil
//		})
//	}
//
// ```
//
// ## Delete
//
// Disassociate DR Protection Group (if associated) before deleting it. Increment value of `disassociateTrigger` property to trigger Disassociate, e.g.
//
// ```go
// package main
//
// import (
//
//	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
//
// )
//
//	func main() {
//		pulumi.Run(func(ctx *pulumi.Context) error {
//			return nil
//		})
//	}
//
// ```
//
// ## Import
//
// DrProtectionGroups can be imported using the `id`, e.g.
//
// ```sh
//
//	$ pulumi import oci:DisasterRecovery/drProtectionGroup:DrProtectionGroup test_dr_protection_group "id"
//
// ```
type DrProtectionGroup struct {
	pulumi.CustomResourceState

	// The details for associating a DR protection group with a peer DR protection group.
	Association DrProtectionGroupAssociationPtrOutput `pulumi:"association"`
	// (Updatable) The OCID of the compartment in which to create the DR protection group.  Example: `ocid1.compartment.oc1..uniqueID`
	CompartmentId pulumi.StringOutput `pulumi:"compartmentId"`
	// (Updatable) Defined tags for this resource. Each key is predefined and scoped to a namespace.  Example: `{"Operations.CostCenter": "42"}`
	DefinedTags pulumi.MapOutput `pulumi:"definedTags"`
	// (Updatable) An optional property when incremented triggers Disassociate. Could be set to any integer value.
	//
	// ** IMPORTANT **
	// Any change to a property that does not support update will force the destruction and recreation of the resource with the new property values
	DisassociateTrigger pulumi.IntPtrOutput `pulumi:"disassociateTrigger"`
	// (Updatable) The display name of the DR protection group.  Example: `EBS PHX Group`
	DisplayName pulumi.StringOutput `pulumi:"displayName"`
	// (Updatable) Simple key-value pair that is applied without any predefined name, type or scope. Exists for cross-compatibility only.  Example: `{"Department": "Finance"}`
	FreeformTags pulumi.MapOutput `pulumi:"freeformTags"`
	// A message describing the DR protection group's current state in more detail.
	LifeCycleDetails pulumi.StringPtrOutput `pulumi:"lifeCycleDetails"`
	// The current sub-state of the DR protection group.
	LifecycleSubState pulumi.StringPtrOutput `pulumi:"lifecycleSubState"`
	// (Updatable) The details for creating an object storage log location for a DR protection group.
	LogLocation DrProtectionGroupLogLocationOutput `pulumi:"logLocation"`
	// (Updatable) A list of DR protection group members.
	Members DrProtectionGroupMemberArrayOutput `pulumi:"members"`
	// The OCID of the peer DR protection group.  Example: `ocid1.drprotectiongroup.oc1..uniqueID`
	PeerId pulumi.StringPtrOutput `pulumi:"peerId"`
	// The region of the peer DR protection group.  Example: `us-ashburn-1`
	PeerRegion pulumi.StringPtrOutput `pulumi:"peerRegion"`
	// The role of the DR protection group.  Example: `STANDBY`
	Role pulumi.StringPtrOutput `pulumi:"role"`
	// The current state of the DR protection group.
	State pulumi.StringPtrOutput `pulumi:"state"`
	// Usage of system tag keys. These predefined keys are scoped to namespaces.  Example: `{"orcl-cloud.free-tier-retained": "true"}`
	SystemTags pulumi.MapOutput `pulumi:"systemTags"`
	// The date and time the DR protection group was created. An RFC3339 formatted datetime string.  Example: `2019-03-29T09:36:42Z`
	TimeCreated pulumi.StringPtrOutput `pulumi:"timeCreated"`
	// The date and time the DR protection group was updated. An RFC3339 formatted datetime string.  Example: `2019-03-29T09:36:42Z`
	TimeUpdated pulumi.StringPtrOutput `pulumi:"timeUpdated"`
}

// NewDrProtectionGroup registers a new resource with the given unique name, arguments, and options.
func NewDrProtectionGroup(ctx *pulumi.Context,
	name string, args *DrProtectionGroupArgs, opts ...pulumi.ResourceOption) (*DrProtectionGroup, error) {
	if args == nil {
		return nil, errors.New("missing one or more required arguments")
	}

	if args.CompartmentId == nil {
		return nil, errors.New("invalid value for required argument 'CompartmentId'")
	}
	if args.DisplayName == nil {
		return nil, errors.New("invalid value for required argument 'DisplayName'")
	}
	if args.LogLocation == nil {
		return nil, errors.New("invalid value for required argument 'LogLocation'")
	}
	opts = internal.PkgResourceDefaultOpts(opts)
	var resource DrProtectionGroup
	err := ctx.RegisterResource("oci:DisasterRecovery/drProtectionGroup:DrProtectionGroup", name, args, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
}

// GetDrProtectionGroup gets an existing DrProtectionGroup resource's state with the given name, ID, and optional
// state properties that are used to uniquely qualify the lookup (nil if not required).
func GetDrProtectionGroup(ctx *pulumi.Context,
	name string, id pulumi.IDInput, state *DrProtectionGroupState, opts ...pulumi.ResourceOption) (*DrProtectionGroup, error) {
	var resource DrProtectionGroup
	err := ctx.ReadResource("oci:DisasterRecovery/drProtectionGroup:DrProtectionGroup", name, id, state, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
}

// Input properties used for looking up and filtering DrProtectionGroup resources.
type drProtectionGroupState struct {
	// The details for associating a DR protection group with a peer DR protection group.
	Association *DrProtectionGroupAssociation `pulumi:"association"`
	// (Updatable) The OCID of the compartment in which to create the DR protection group.  Example: `ocid1.compartment.oc1..uniqueID`
	CompartmentId *string `pulumi:"compartmentId"`
	// (Updatable) Defined tags for this resource. Each key is predefined and scoped to a namespace.  Example: `{"Operations.CostCenter": "42"}`
	DefinedTags map[string]interface{} `pulumi:"definedTags"`
	// (Updatable) An optional property when incremented triggers Disassociate. Could be set to any integer value.
	//
	// ** IMPORTANT **
	// Any change to a property that does not support update will force the destruction and recreation of the resource with the new property values
	DisassociateTrigger *int `pulumi:"disassociateTrigger"`
	// (Updatable) The display name of the DR protection group.  Example: `EBS PHX Group`
	DisplayName *string `pulumi:"displayName"`
	// (Updatable) Simple key-value pair that is applied without any predefined name, type or scope. Exists for cross-compatibility only.  Example: `{"Department": "Finance"}`
	FreeformTags map[string]interface{} `pulumi:"freeformTags"`
	// A message describing the DR protection group's current state in more detail.
	LifeCycleDetails *string `pulumi:"lifeCycleDetails"`
	// The current sub-state of the DR protection group.
	LifecycleSubState *string `pulumi:"lifecycleSubState"`
	// (Updatable) The details for creating an object storage log location for a DR protection group.
	LogLocation *DrProtectionGroupLogLocation `pulumi:"logLocation"`
	// (Updatable) A list of DR protection group members.
	Members []DrProtectionGroupMember `pulumi:"members"`
	// The OCID of the peer DR protection group.  Example: `ocid1.drprotectiongroup.oc1..uniqueID`
	PeerId *string `pulumi:"peerId"`
	// The region of the peer DR protection group.  Example: `us-ashburn-1`
	PeerRegion *string `pulumi:"peerRegion"`
	// The role of the DR protection group.  Example: `STANDBY`
	Role *string `pulumi:"role"`
	// The current state of the DR protection group.
	State *string `pulumi:"state"`
	// Usage of system tag keys. These predefined keys are scoped to namespaces.  Example: `{"orcl-cloud.free-tier-retained": "true"}`
	SystemTags map[string]interface{} `pulumi:"systemTags"`
	// The date and time the DR protection group was created. An RFC3339 formatted datetime string.  Example: `2019-03-29T09:36:42Z`
	TimeCreated *string `pulumi:"timeCreated"`
	// The date and time the DR protection group was updated. An RFC3339 formatted datetime string.  Example: `2019-03-29T09:36:42Z`
	TimeUpdated *string `pulumi:"timeUpdated"`
}

type DrProtectionGroupState struct {
	// The details for associating a DR protection group with a peer DR protection group.
	Association DrProtectionGroupAssociationPtrInput
	// (Updatable) The OCID of the compartment in which to create the DR protection group.  Example: `ocid1.compartment.oc1..uniqueID`
	CompartmentId pulumi.StringPtrInput
	// (Updatable) Defined tags for this resource. Each key is predefined and scoped to a namespace.  Example: `{"Operations.CostCenter": "42"}`
	DefinedTags pulumi.MapInput
	// (Updatable) An optional property when incremented triggers Disassociate. Could be set to any integer value.
	//
	// ** IMPORTANT **
	// Any change to a property that does not support update will force the destruction and recreation of the resource with the new property values
	DisassociateTrigger pulumi.IntPtrInput
	// (Updatable) The display name of the DR protection group.  Example: `EBS PHX Group`
	DisplayName pulumi.StringPtrInput
	// (Updatable) Simple key-value pair that is applied without any predefined name, type or scope. Exists for cross-compatibility only.  Example: `{"Department": "Finance"}`
	FreeformTags pulumi.MapInput
	// A message describing the DR protection group's current state in more detail.
	LifeCycleDetails pulumi.StringPtrInput
	// The current sub-state of the DR protection group.
	LifecycleSubState pulumi.StringPtrInput
	// (Updatable) The details for creating an object storage log location for a DR protection group.
	LogLocation DrProtectionGroupLogLocationPtrInput
	// (Updatable) A list of DR protection group members.
	Members DrProtectionGroupMemberArrayInput
	// The OCID of the peer DR protection group.  Example: `ocid1.drprotectiongroup.oc1..uniqueID`
	PeerId pulumi.StringPtrInput
	// The region of the peer DR protection group.  Example: `us-ashburn-1`
	PeerRegion pulumi.StringPtrInput
	// The role of the DR protection group.  Example: `STANDBY`
	Role pulumi.StringPtrInput
	// The current state of the DR protection group.
	State pulumi.StringPtrInput
	// Usage of system tag keys. These predefined keys are scoped to namespaces.  Example: `{"orcl-cloud.free-tier-retained": "true"}`
	SystemTags pulumi.MapInput
	// The date and time the DR protection group was created. An RFC3339 formatted datetime string.  Example: `2019-03-29T09:36:42Z`
	TimeCreated pulumi.StringPtrInput
	// The date and time the DR protection group was updated. An RFC3339 formatted datetime string.  Example: `2019-03-29T09:36:42Z`
	TimeUpdated pulumi.StringPtrInput
}

func (DrProtectionGroupState) ElementType() reflect.Type {
	return reflect.TypeOf((*drProtectionGroupState)(nil)).Elem()
}

type drProtectionGroupArgs struct {
	// The details for associating a DR protection group with a peer DR protection group.
	Association *DrProtectionGroupAssociation `pulumi:"association"`
	// (Updatable) The OCID of the compartment in which to create the DR protection group.  Example: `ocid1.compartment.oc1..uniqueID`
	CompartmentId string `pulumi:"compartmentId"`
	// (Updatable) Defined tags for this resource. Each key is predefined and scoped to a namespace.  Example: `{"Operations.CostCenter": "42"}`
	DefinedTags map[string]interface{} `pulumi:"definedTags"`
	// (Updatable) An optional property when incremented triggers Disassociate. Could be set to any integer value.
	//
	// ** IMPORTANT **
	// Any change to a property that does not support update will force the destruction and recreation of the resource with the new property values
	DisassociateTrigger *int `pulumi:"disassociateTrigger"`
	// (Updatable) The display name of the DR protection group.  Example: `EBS PHX Group`
	DisplayName string `pulumi:"displayName"`
	// (Updatable) Simple key-value pair that is applied without any predefined name, type or scope. Exists for cross-compatibility only.  Example: `{"Department": "Finance"}`
	FreeformTags map[string]interface{} `pulumi:"freeformTags"`
	// (Updatable) The details for creating an object storage log location for a DR protection group.
	LogLocation DrProtectionGroupLogLocation `pulumi:"logLocation"`
	// (Updatable) A list of DR protection group members.
	Members []DrProtectionGroupMember `pulumi:"members"`
}

// The set of arguments for constructing a DrProtectionGroup resource.
type DrProtectionGroupArgs struct {
	// The details for associating a DR protection group with a peer DR protection group.
	Association DrProtectionGroupAssociationPtrInput
	// (Updatable) The OCID of the compartment in which to create the DR protection group.  Example: `ocid1.compartment.oc1..uniqueID`
	CompartmentId pulumi.StringInput
	// (Updatable) Defined tags for this resource. Each key is predefined and scoped to a namespace.  Example: `{"Operations.CostCenter": "42"}`
	DefinedTags pulumi.MapInput
	// (Updatable) An optional property when incremented triggers Disassociate. Could be set to any integer value.
	//
	// ** IMPORTANT **
	// Any change to a property that does not support update will force the destruction and recreation of the resource with the new property values
	DisassociateTrigger pulumi.IntPtrInput
	// (Updatable) The display name of the DR protection group.  Example: `EBS PHX Group`
	DisplayName pulumi.StringInput
	// (Updatable) Simple key-value pair that is applied without any predefined name, type or scope. Exists for cross-compatibility only.  Example: `{"Department": "Finance"}`
	FreeformTags pulumi.MapInput
	// (Updatable) The details for creating an object storage log location for a DR protection group.
	LogLocation DrProtectionGroupLogLocationInput
	// (Updatable) A list of DR protection group members.
	Members DrProtectionGroupMemberArrayInput
}

func (DrProtectionGroupArgs) ElementType() reflect.Type {
	return reflect.TypeOf((*drProtectionGroupArgs)(nil)).Elem()
}

type DrProtectionGroupInput interface {
	pulumi.Input

	ToDrProtectionGroupOutput() DrProtectionGroupOutput
	ToDrProtectionGroupOutputWithContext(ctx context.Context) DrProtectionGroupOutput
}

func (*DrProtectionGroup) ElementType() reflect.Type {
	return reflect.TypeOf((**DrProtectionGroup)(nil)).Elem()
}

func (i *DrProtectionGroup) ToDrProtectionGroupOutput() DrProtectionGroupOutput {
	return i.ToDrProtectionGroupOutputWithContext(context.Background())
}

func (i *DrProtectionGroup) ToDrProtectionGroupOutputWithContext(ctx context.Context) DrProtectionGroupOutput {
	return pulumi.ToOutputWithContext(ctx, i).(DrProtectionGroupOutput)
}

// DrProtectionGroupArrayInput is an input type that accepts DrProtectionGroupArray and DrProtectionGroupArrayOutput values.
// You can construct a concrete instance of `DrProtectionGroupArrayInput` via:
//
//	DrProtectionGroupArray{ DrProtectionGroupArgs{...} }
type DrProtectionGroupArrayInput interface {
	pulumi.Input

	ToDrProtectionGroupArrayOutput() DrProtectionGroupArrayOutput
	ToDrProtectionGroupArrayOutputWithContext(context.Context) DrProtectionGroupArrayOutput
}

type DrProtectionGroupArray []DrProtectionGroupInput

func (DrProtectionGroupArray) ElementType() reflect.Type {
	return reflect.TypeOf((*[]*DrProtectionGroup)(nil)).Elem()
}

func (i DrProtectionGroupArray) ToDrProtectionGroupArrayOutput() DrProtectionGroupArrayOutput {
	return i.ToDrProtectionGroupArrayOutputWithContext(context.Background())
}

func (i DrProtectionGroupArray) ToDrProtectionGroupArrayOutputWithContext(ctx context.Context) DrProtectionGroupArrayOutput {
	return pulumi.ToOutputWithContext(ctx, i).(DrProtectionGroupArrayOutput)
}

// DrProtectionGroupMapInput is an input type that accepts DrProtectionGroupMap and DrProtectionGroupMapOutput values.
// You can construct a concrete instance of `DrProtectionGroupMapInput` via:
//
//	DrProtectionGroupMap{ "key": DrProtectionGroupArgs{...} }
type DrProtectionGroupMapInput interface {
	pulumi.Input

	ToDrProtectionGroupMapOutput() DrProtectionGroupMapOutput
	ToDrProtectionGroupMapOutputWithContext(context.Context) DrProtectionGroupMapOutput
}

type DrProtectionGroupMap map[string]DrProtectionGroupInput

func (DrProtectionGroupMap) ElementType() reflect.Type {
	return reflect.TypeOf((*map[string]*DrProtectionGroup)(nil)).Elem()
}

func (i DrProtectionGroupMap) ToDrProtectionGroupMapOutput() DrProtectionGroupMapOutput {
	return i.ToDrProtectionGroupMapOutputWithContext(context.Background())
}

func (i DrProtectionGroupMap) ToDrProtectionGroupMapOutputWithContext(ctx context.Context) DrProtectionGroupMapOutput {
	return pulumi.ToOutputWithContext(ctx, i).(DrProtectionGroupMapOutput)
}

type DrProtectionGroupOutput struct{ *pulumi.OutputState }

func (DrProtectionGroupOutput) ElementType() reflect.Type {
	return reflect.TypeOf((**DrProtectionGroup)(nil)).Elem()
}

func (o DrProtectionGroupOutput) ToDrProtectionGroupOutput() DrProtectionGroupOutput {
	return o
}

func (o DrProtectionGroupOutput) ToDrProtectionGroupOutputWithContext(ctx context.Context) DrProtectionGroupOutput {
	return o
}

// The details for associating a DR protection group with a peer DR protection group.
func (o DrProtectionGroupOutput) Association() DrProtectionGroupAssociationPtrOutput {
	return o.ApplyT(func(v *DrProtectionGroup) DrProtectionGroupAssociationPtrOutput { return v.Association }).(DrProtectionGroupAssociationPtrOutput)
}

// (Updatable) The OCID of the compartment in which to create the DR protection group.  Example: `ocid1.compartment.oc1..uniqueID`
func (o DrProtectionGroupOutput) CompartmentId() pulumi.StringOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.StringOutput { return v.CompartmentId }).(pulumi.StringOutput)
}

// (Updatable) Defined tags for this resource. Each key is predefined and scoped to a namespace.  Example: `{"Operations.CostCenter": "42"}`
func (o DrProtectionGroupOutput) DefinedTags() pulumi.MapOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.MapOutput { return v.DefinedTags }).(pulumi.MapOutput)
}

// (Updatable) An optional property when incremented triggers Disassociate. Could be set to any integer value.
//
// ** IMPORTANT **
// Any change to a property that does not support update will force the destruction and recreation of the resource with the new property values
func (o DrProtectionGroupOutput) DisassociateTrigger() pulumi.IntPtrOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.IntPtrOutput { return v.DisassociateTrigger }).(pulumi.IntPtrOutput)
}

// (Updatable) The display name of the DR protection group.  Example: `EBS PHX Group`
func (o DrProtectionGroupOutput) DisplayName() pulumi.StringOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.StringOutput { return v.DisplayName }).(pulumi.StringOutput)
}

// (Updatable) Simple key-value pair that is applied without any predefined name, type or scope. Exists for cross-compatibility only.  Example: `{"Department": "Finance"}`
func (o DrProtectionGroupOutput) FreeformTags() pulumi.MapOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.MapOutput { return v.FreeformTags }).(pulumi.MapOutput)
}

// A message describing the DR protection group's current state in more detail.
func (o DrProtectionGroupOutput) LifeCycleDetails() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.StringPtrOutput { return v.LifeCycleDetails }).(pulumi.StringPtrOutput)
}

// The current sub-state of the DR protection group.
func (o DrProtectionGroupOutput) LifecycleSubState() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.StringPtrOutput { return v.LifecycleSubState }).(pulumi.StringPtrOutput)
}

// (Updatable) The details for creating an object storage log location for a DR protection group.
func (o DrProtectionGroupOutput) LogLocation() DrProtectionGroupLogLocationOutput {
	return o.ApplyT(func(v *DrProtectionGroup) DrProtectionGroupLogLocationOutput { return v.LogLocation }).(DrProtectionGroupLogLocationOutput)
}

// (Updatable) A list of DR protection group members.
func (o DrProtectionGroupOutput) Members() DrProtectionGroupMemberArrayOutput {
	return o.ApplyT(func(v *DrProtectionGroup) DrProtectionGroupMemberArrayOutput { return v.Members }).(DrProtectionGroupMemberArrayOutput)
}

// The OCID of the peer DR protection group.  Example: `ocid1.drprotectiongroup.oc1..uniqueID`
func (o DrProtectionGroupOutput) PeerId() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.StringPtrOutput { return v.PeerId }).(pulumi.StringPtrOutput)
}

// The region of the peer DR protection group.  Example: `us-ashburn-1`
func (o DrProtectionGroupOutput) PeerRegion() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.StringPtrOutput { return v.PeerRegion }).(pulumi.StringPtrOutput)
}

// The role of the DR protection group.  Example: `STANDBY`
func (o DrProtectionGroupOutput) Role() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.StringPtrOutput { return v.Role }).(pulumi.StringPtrOutput)
}

// The current state of the DR protection group.
func (o DrProtectionGroupOutput) State() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.StringPtrOutput { return v.State }).(pulumi.StringPtrOutput)
}

// Usage of system tag keys. These predefined keys are scoped to namespaces.  Example: `{"orcl-cloud.free-tier-retained": "true"}`
func (o DrProtectionGroupOutput) SystemTags() pulumi.MapOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.MapOutput { return v.SystemTags }).(pulumi.MapOutput)
}

// The date and time the DR protection group was created. An RFC3339 formatted datetime string.  Example: `2019-03-29T09:36:42Z`
func (o DrProtectionGroupOutput) TimeCreated() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.StringPtrOutput { return v.TimeCreated }).(pulumi.StringPtrOutput)
}

// The date and time the DR protection group was updated. An RFC3339 formatted datetime string.  Example: `2019-03-29T09:36:42Z`
func (o DrProtectionGroupOutput) TimeUpdated() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *DrProtectionGroup) pulumi.StringPtrOutput { return v.TimeUpdated }).(pulumi.StringPtrOutput)
}

type DrProtectionGroupArrayOutput struct{ *pulumi.OutputState }

func (DrProtectionGroupArrayOutput) ElementType() reflect.Type {
	return reflect.TypeOf((*[]*DrProtectionGroup)(nil)).Elem()
}

func (o DrProtectionGroupArrayOutput) ToDrProtectionGroupArrayOutput() DrProtectionGroupArrayOutput {
	return o
}

func (o DrProtectionGroupArrayOutput) ToDrProtectionGroupArrayOutputWithContext(ctx context.Context) DrProtectionGroupArrayOutput {
	return o
}

func (o DrProtectionGroupArrayOutput) Index(i pulumi.IntInput) DrProtectionGroupOutput {
	return pulumi.All(o, i).ApplyT(func(vs []interface{}) *DrProtectionGroup {
		return vs[0].([]*DrProtectionGroup)[vs[1].(int)]
	}).(DrProtectionGroupOutput)
}

type DrProtectionGroupMapOutput struct{ *pulumi.OutputState }

func (DrProtectionGroupMapOutput) ElementType() reflect.Type {
	return reflect.TypeOf((*map[string]*DrProtectionGroup)(nil)).Elem()
}

func (o DrProtectionGroupMapOutput) ToDrProtectionGroupMapOutput() DrProtectionGroupMapOutput {
	return o
}

func (o DrProtectionGroupMapOutput) ToDrProtectionGroupMapOutputWithContext(ctx context.Context) DrProtectionGroupMapOutput {
	return o
}

func (o DrProtectionGroupMapOutput) MapIndex(k pulumi.StringInput) DrProtectionGroupOutput {
	return pulumi.All(o, k).ApplyT(func(vs []interface{}) *DrProtectionGroup {
		return vs[0].(map[string]*DrProtectionGroup)[vs[1].(string)]
	}).(DrProtectionGroupOutput)
}

func init() {
	pulumi.RegisterInputType(reflect.TypeOf((*DrProtectionGroupInput)(nil)).Elem(), &DrProtectionGroup{})
	pulumi.RegisterInputType(reflect.TypeOf((*DrProtectionGroupArrayInput)(nil)).Elem(), DrProtectionGroupArray{})
	pulumi.RegisterInputType(reflect.TypeOf((*DrProtectionGroupMapInput)(nil)).Elem(), DrProtectionGroupMap{})
	pulumi.RegisterOutputType(DrProtectionGroupOutput{})
	pulumi.RegisterOutputType(DrProtectionGroupArrayOutput{})
	pulumi.RegisterOutputType(DrProtectionGroupMapOutput{})
}
