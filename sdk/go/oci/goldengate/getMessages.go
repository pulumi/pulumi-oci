// Code generated by the Pulumi Terraform Bridge (tfgen) Tool DO NOT EDIT.
// *** WARNING: Do not edit by hand unless you're certain you know what you are doing! ***

package goldengate

import (
	"context"
	"reflect"

	"github.com/pulumi/pulumi-oci/sdk/v2/go/oci/internal"
	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
)

// This data source provides the list of Messages in Oracle Cloud Infrastructure Golden Gate service.
//
// Lists the DeploymentMessages for a deployment. The sorting order is not important. By default first will be Upgrade message, next Exception message and then Storage Utilization message.
//
// ## Example Usage
//
// ```go
// package main
//
// import (
//
//	"github.com/pulumi/pulumi-oci/sdk/v2/go/oci/GoldenGate"
//	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
//
// )
//
//	func main() {
//		pulumi.Run(func(ctx *pulumi.Context) error {
//			_, err := GoldenGate.GetMessages(ctx, &goldengate.GetMessagesArgs{
//				DeploymentId: testDeployment.Id,
//			}, nil)
//			if err != nil {
//				return err
//			}
//			return nil
//		})
//	}
//
// ```
func GetMessages(ctx *pulumi.Context, args *GetMessagesArgs, opts ...pulumi.InvokeOption) (*GetMessagesResult, error) {
	opts = internal.PkgInvokeDefaultOpts(opts)
	var rv GetMessagesResult
	err := ctx.Invoke("oci:GoldenGate/getMessages:getMessages", args, &rv, opts...)
	if err != nil {
		return nil, err
	}
	return &rv, nil
}

// A collection of arguments for invoking getMessages.
type GetMessagesArgs struct {
	// A unique Deployment identifier.
	DeploymentId string              `pulumi:"deploymentId"`
	Filters      []GetMessagesFilter `pulumi:"filters"`
}

// A collection of values returned by getMessages.
type GetMessagesResult struct {
	DeploymentId string `pulumi:"deploymentId"`
	// The list of deployment_messages_collection.
	DeploymentMessagesCollections []GetMessagesDeploymentMessagesCollection `pulumi:"deploymentMessagesCollections"`
	Filters                       []GetMessagesFilter                       `pulumi:"filters"`
	// The provider-assigned unique ID for this managed resource.
	Id string `pulumi:"id"`
}

func GetMessagesOutput(ctx *pulumi.Context, args GetMessagesOutputArgs, opts ...pulumi.InvokeOption) GetMessagesResultOutput {
	return pulumi.ToOutputWithContext(context.Background(), args).
		ApplyT(func(v interface{}) (GetMessagesResultOutput, error) {
			args := v.(GetMessagesArgs)
			opts = internal.PkgInvokeDefaultOpts(opts)
			var rv GetMessagesResult
			secret, err := ctx.InvokePackageRaw("oci:GoldenGate/getMessages:getMessages", args, &rv, "", opts...)
			if err != nil {
				return GetMessagesResultOutput{}, err
			}

			output := pulumi.ToOutput(rv).(GetMessagesResultOutput)
			if secret {
				return pulumi.ToSecret(output).(GetMessagesResultOutput), nil
			}
			return output, nil
		}).(GetMessagesResultOutput)
}

// A collection of arguments for invoking getMessages.
type GetMessagesOutputArgs struct {
	// A unique Deployment identifier.
	DeploymentId pulumi.StringInput          `pulumi:"deploymentId"`
	Filters      GetMessagesFilterArrayInput `pulumi:"filters"`
}

func (GetMessagesOutputArgs) ElementType() reflect.Type {
	return reflect.TypeOf((*GetMessagesArgs)(nil)).Elem()
}

// A collection of values returned by getMessages.
type GetMessagesResultOutput struct{ *pulumi.OutputState }

func (GetMessagesResultOutput) ElementType() reflect.Type {
	return reflect.TypeOf((*GetMessagesResult)(nil)).Elem()
}

func (o GetMessagesResultOutput) ToGetMessagesResultOutput() GetMessagesResultOutput {
	return o
}

func (o GetMessagesResultOutput) ToGetMessagesResultOutputWithContext(ctx context.Context) GetMessagesResultOutput {
	return o
}

func (o GetMessagesResultOutput) DeploymentId() pulumi.StringOutput {
	return o.ApplyT(func(v GetMessagesResult) string { return v.DeploymentId }).(pulumi.StringOutput)
}

// The list of deployment_messages_collection.
func (o GetMessagesResultOutput) DeploymentMessagesCollections() GetMessagesDeploymentMessagesCollectionArrayOutput {
	return o.ApplyT(func(v GetMessagesResult) []GetMessagesDeploymentMessagesCollection {
		return v.DeploymentMessagesCollections
	}).(GetMessagesDeploymentMessagesCollectionArrayOutput)
}

func (o GetMessagesResultOutput) Filters() GetMessagesFilterArrayOutput {
	return o.ApplyT(func(v GetMessagesResult) []GetMessagesFilter { return v.Filters }).(GetMessagesFilterArrayOutput)
}

// The provider-assigned unique ID for this managed resource.
func (o GetMessagesResultOutput) Id() pulumi.StringOutput {
	return o.ApplyT(func(v GetMessagesResult) string { return v.Id }).(pulumi.StringOutput)
}

func init() {
	pulumi.RegisterOutputType(GetMessagesResultOutput{})
}
