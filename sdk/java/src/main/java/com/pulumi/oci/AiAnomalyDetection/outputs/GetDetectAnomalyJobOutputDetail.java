// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.oci.AiAnomalyDetection.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;

@CustomType
public final class GetDetectAnomalyJobOutputDetail {
    /**
     * @return Object Storage bucket name.
     * 
     */
    private String bucket;
    /**
     * @return Object Storage namespace.
     * 
     */
    private String namespace;
    /**
     * @return The type of output location Allowed values are:
     * * `OBJECT_STORAGE`: Object store output location.
     * 
     */
    private String outputType;
    /**
     * @return Object Storage folder name.
     * 
     */
    private String prefix;

    private GetDetectAnomalyJobOutputDetail() {}
    /**
     * @return Object Storage bucket name.
     * 
     */
    public String bucket() {
        return this.bucket;
    }
    /**
     * @return Object Storage namespace.
     * 
     */
    public String namespace() {
        return this.namespace;
    }
    /**
     * @return The type of output location Allowed values are:
     * * `OBJECT_STORAGE`: Object store output location.
     * 
     */
    public String outputType() {
        return this.outputType;
    }
    /**
     * @return Object Storage folder name.
     * 
     */
    public String prefix() {
        return this.prefix;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GetDetectAnomalyJobOutputDetail defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private String bucket;
        private String namespace;
        private String outputType;
        private String prefix;
        public Builder() {}
        public Builder(GetDetectAnomalyJobOutputDetail defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.bucket = defaults.bucket;
    	      this.namespace = defaults.namespace;
    	      this.outputType = defaults.outputType;
    	      this.prefix = defaults.prefix;
        }

        @CustomType.Setter
        public Builder bucket(String bucket) {
            if (bucket == null) {
              throw new MissingRequiredPropertyException("GetDetectAnomalyJobOutputDetail", "bucket");
            }
            this.bucket = bucket;
            return this;
        }
        @CustomType.Setter
        public Builder namespace(String namespace) {
            if (namespace == null) {
              throw new MissingRequiredPropertyException("GetDetectAnomalyJobOutputDetail", "namespace");
            }
            this.namespace = namespace;
            return this;
        }
        @CustomType.Setter
        public Builder outputType(String outputType) {
            if (outputType == null) {
              throw new MissingRequiredPropertyException("GetDetectAnomalyJobOutputDetail", "outputType");
            }
            this.outputType = outputType;
            return this;
        }
        @CustomType.Setter
        public Builder prefix(String prefix) {
            if (prefix == null) {
              throw new MissingRequiredPropertyException("GetDetectAnomalyJobOutputDetail", "prefix");
            }
            this.prefix = prefix;
            return this;
        }
        public GetDetectAnomalyJobOutputDetail build() {
            final var _resultValue = new GetDetectAnomalyJobOutputDetail();
            _resultValue.bucket = bucket;
            _resultValue.namespace = namespace;
            _resultValue.outputType = outputType;
            _resultValue.prefix = prefix;
            return _resultValue;
        }
    }
}
