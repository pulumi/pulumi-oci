// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.oci.Artifacts.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;


public final class GetGenericArtifactArgs extends com.pulumi.resources.InvokeArgs {

    public static final GetGenericArtifactArgs Empty = new GetGenericArtifactArgs();

    /**
     * The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the artifact.  Example: `ocid1.genericartifact.oc1..exampleuniqueID`
     * 
     */
    @Import(name="artifactId", required=true)
    private Output<String> artifactId;

    /**
     * @return The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the artifact.  Example: `ocid1.genericartifact.oc1..exampleuniqueID`
     * 
     */
    public Output<String> artifactId() {
        return this.artifactId;
    }

    private GetGenericArtifactArgs() {}

    private GetGenericArtifactArgs(GetGenericArtifactArgs $) {
        this.artifactId = $.artifactId;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetGenericArtifactArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetGenericArtifactArgs $;

        public Builder() {
            $ = new GetGenericArtifactArgs();
        }

        public Builder(GetGenericArtifactArgs defaults) {
            $ = new GetGenericArtifactArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param artifactId The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the artifact.  Example: `ocid1.genericartifact.oc1..exampleuniqueID`
         * 
         * @return builder
         * 
         */
        public Builder artifactId(Output<String> artifactId) {
            $.artifactId = artifactId;
            return this;
        }

        /**
         * @param artifactId The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the artifact.  Example: `ocid1.genericartifact.oc1..exampleuniqueID`
         * 
         * @return builder
         * 
         */
        public Builder artifactId(String artifactId) {
            return artifactId(Output.of(artifactId));
        }

        public GetGenericArtifactArgs build() {
            if ($.artifactId == null) {
                throw new MissingRequiredPropertyException("GetGenericArtifactArgs", "artifactId");
            }
            return $;
        }
    }

}
