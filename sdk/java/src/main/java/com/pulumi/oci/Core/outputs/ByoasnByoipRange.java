// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.oci.Core.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class ByoasnByoipRange {
    /**
     * @return The as path prepend length.
     * 
     */
    private @Nullable Integer asPathPrependLength;
    /**
     * @return The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the `ByoipRange` resource to which the CIDR block belongs.
     * 
     */
    private @Nullable String byoipRangeId;
    /**
     * @return The BYOIP CIDR block range or subrange allocated to an IP pool. This could be all or part of a BYOIP CIDR block.
     * 
     */
    private @Nullable String cidrBlock;
    /**
     * @return The IPv6 prefix being imported to the Oracle cloud. This prefix must be /48 or larger, and can  be subdivided into sub-ranges used across multiple VCNs. A BYOIPv6 prefix can be assigned across multiple VCNs, and each VCN must be /64 or larger. You may specify a ULA or private IPv6 prefix of /64 or larger to use in the VCN. IPv6-enabled subnets will remain a fixed /64 in size.
     * 
     */
    private @Nullable String ipv6cidrBlock;

    private ByoasnByoipRange() {}
    /**
     * @return The as path prepend length.
     * 
     */
    public Optional<Integer> asPathPrependLength() {
        return Optional.ofNullable(this.asPathPrependLength);
    }
    /**
     * @return The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the `ByoipRange` resource to which the CIDR block belongs.
     * 
     */
    public Optional<String> byoipRangeId() {
        return Optional.ofNullable(this.byoipRangeId);
    }
    /**
     * @return The BYOIP CIDR block range or subrange allocated to an IP pool. This could be all or part of a BYOIP CIDR block.
     * 
     */
    public Optional<String> cidrBlock() {
        return Optional.ofNullable(this.cidrBlock);
    }
    /**
     * @return The IPv6 prefix being imported to the Oracle cloud. This prefix must be /48 or larger, and can  be subdivided into sub-ranges used across multiple VCNs. A BYOIPv6 prefix can be assigned across multiple VCNs, and each VCN must be /64 or larger. You may specify a ULA or private IPv6 prefix of /64 or larger to use in the VCN. IPv6-enabled subnets will remain a fixed /64 in size.
     * 
     */
    public Optional<String> ipv6cidrBlock() {
        return Optional.ofNullable(this.ipv6cidrBlock);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(ByoasnByoipRange defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable Integer asPathPrependLength;
        private @Nullable String byoipRangeId;
        private @Nullable String cidrBlock;
        private @Nullable String ipv6cidrBlock;
        public Builder() {}
        public Builder(ByoasnByoipRange defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.asPathPrependLength = defaults.asPathPrependLength;
    	      this.byoipRangeId = defaults.byoipRangeId;
    	      this.cidrBlock = defaults.cidrBlock;
    	      this.ipv6cidrBlock = defaults.ipv6cidrBlock;
        }

        @CustomType.Setter
        public Builder asPathPrependLength(@Nullable Integer asPathPrependLength) {

            this.asPathPrependLength = asPathPrependLength;
            return this;
        }
        @CustomType.Setter
        public Builder byoipRangeId(@Nullable String byoipRangeId) {

            this.byoipRangeId = byoipRangeId;
            return this;
        }
        @CustomType.Setter
        public Builder cidrBlock(@Nullable String cidrBlock) {

            this.cidrBlock = cidrBlock;
            return this;
        }
        @CustomType.Setter
        public Builder ipv6cidrBlock(@Nullable String ipv6cidrBlock) {

            this.ipv6cidrBlock = ipv6cidrBlock;
            return this;
        }
        public ByoasnByoipRange build() {
            final var _resultValue = new ByoasnByoipRange();
            _resultValue.asPathPrependLength = asPathPrependLength;
            _resultValue.byoipRangeId = byoipRangeId;
            _resultValue.cidrBlock = cidrBlock;
            _resultValue.ipv6cidrBlock = ipv6cidrBlock;
            return _resultValue;
        }
    }
}
