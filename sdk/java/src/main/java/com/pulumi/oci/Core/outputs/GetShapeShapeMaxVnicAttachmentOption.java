// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.oci.Core.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.Double;
import java.lang.Integer;
import java.util.Objects;

@CustomType
public final class GetShapeShapeMaxVnicAttachmentOption {
    private Double defaultPerOcpu;
    private Double max;
    private Integer min;

    private GetShapeShapeMaxVnicAttachmentOption() {}
    public Double defaultPerOcpu() {
        return this.defaultPerOcpu;
    }
    public Double max() {
        return this.max;
    }
    public Integer min() {
        return this.min;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GetShapeShapeMaxVnicAttachmentOption defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private Double defaultPerOcpu;
        private Double max;
        private Integer min;
        public Builder() {}
        public Builder(GetShapeShapeMaxVnicAttachmentOption defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.defaultPerOcpu = defaults.defaultPerOcpu;
    	      this.max = defaults.max;
    	      this.min = defaults.min;
        }

        @CustomType.Setter
        public Builder defaultPerOcpu(Double defaultPerOcpu) {
            if (defaultPerOcpu == null) {
              throw new MissingRequiredPropertyException("GetShapeShapeMaxVnicAttachmentOption", "defaultPerOcpu");
            }
            this.defaultPerOcpu = defaultPerOcpu;
            return this;
        }
        @CustomType.Setter
        public Builder max(Double max) {
            if (max == null) {
              throw new MissingRequiredPropertyException("GetShapeShapeMaxVnicAttachmentOption", "max");
            }
            this.max = max;
            return this;
        }
        @CustomType.Setter
        public Builder min(Integer min) {
            if (min == null) {
              throw new MissingRequiredPropertyException("GetShapeShapeMaxVnicAttachmentOption", "min");
            }
            this.min = min;
            return this;
        }
        public GetShapeShapeMaxVnicAttachmentOption build() {
            final var _resultValue = new GetShapeShapeMaxVnicAttachmentOption();
            _resultValue.defaultPerOcpu = defaultPerOcpu;
            _resultValue.max = max;
            _resultValue.min = min;
            return _resultValue;
        }
    }
}
