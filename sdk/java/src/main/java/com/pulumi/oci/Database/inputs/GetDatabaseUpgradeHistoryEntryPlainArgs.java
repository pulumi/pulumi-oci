// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.oci.Database.inputs;

import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;


public final class GetDatabaseUpgradeHistoryEntryPlainArgs extends com.pulumi.resources.InvokeArgs {

    public static final GetDatabaseUpgradeHistoryEntryPlainArgs Empty = new GetDatabaseUpgradeHistoryEntryPlainArgs();

    /**
     * The database [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm).
     * 
     */
    @Import(name="databaseId", required=true)
    private String databaseId;

    /**
     * @return The database [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm).
     * 
     */
    public String databaseId() {
        return this.databaseId;
    }

    /**
     * The database/db system upgrade History [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm).
     * 
     */
    @Import(name="upgradeHistoryEntryId", required=true)
    private String upgradeHistoryEntryId;

    /**
     * @return The database/db system upgrade History [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm).
     * 
     */
    public String upgradeHistoryEntryId() {
        return this.upgradeHistoryEntryId;
    }

    private GetDatabaseUpgradeHistoryEntryPlainArgs() {}

    private GetDatabaseUpgradeHistoryEntryPlainArgs(GetDatabaseUpgradeHistoryEntryPlainArgs $) {
        this.databaseId = $.databaseId;
        this.upgradeHistoryEntryId = $.upgradeHistoryEntryId;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetDatabaseUpgradeHistoryEntryPlainArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetDatabaseUpgradeHistoryEntryPlainArgs $;

        public Builder() {
            $ = new GetDatabaseUpgradeHistoryEntryPlainArgs();
        }

        public Builder(GetDatabaseUpgradeHistoryEntryPlainArgs defaults) {
            $ = new GetDatabaseUpgradeHistoryEntryPlainArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param databaseId The database [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm).
         * 
         * @return builder
         * 
         */
        public Builder databaseId(String databaseId) {
            $.databaseId = databaseId;
            return this;
        }

        /**
         * @param upgradeHistoryEntryId The database/db system upgrade History [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm).
         * 
         * @return builder
         * 
         */
        public Builder upgradeHistoryEntryId(String upgradeHistoryEntryId) {
            $.upgradeHistoryEntryId = upgradeHistoryEntryId;
            return this;
        }

        public GetDatabaseUpgradeHistoryEntryPlainArgs build() {
            if ($.databaseId == null) {
                throw new MissingRequiredPropertyException("GetDatabaseUpgradeHistoryEntryPlainArgs", "databaseId");
            }
            if ($.upgradeHistoryEntryId == null) {
                throw new MissingRequiredPropertyException("GetDatabaseUpgradeHistoryEntryPlainArgs", "upgradeHistoryEntryId");
            }
            return $;
        }
    }

}
