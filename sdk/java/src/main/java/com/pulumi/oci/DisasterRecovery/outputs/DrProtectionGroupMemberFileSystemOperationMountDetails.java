// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.oci.DisasterRecovery.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class DrProtectionGroupMemberFileSystemOperationMountDetails {
    /**
     * @return (Updatable) The OCID of the mount target.  Example: `ocid1.mounttarget.oc1..uniqueID`
     * 
     */
    private @Nullable String mountTargetId;

    private DrProtectionGroupMemberFileSystemOperationMountDetails() {}
    /**
     * @return (Updatable) The OCID of the mount target.  Example: `ocid1.mounttarget.oc1..uniqueID`
     * 
     */
    public Optional<String> mountTargetId() {
        return Optional.ofNullable(this.mountTargetId);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(DrProtectionGroupMemberFileSystemOperationMountDetails defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable String mountTargetId;
        public Builder() {}
        public Builder(DrProtectionGroupMemberFileSystemOperationMountDetails defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.mountTargetId = defaults.mountTargetId;
        }

        @CustomType.Setter
        public Builder mountTargetId(@Nullable String mountTargetId) {
            this.mountTargetId = mountTargetId;
            return this;
        }
        public DrProtectionGroupMemberFileSystemOperationMountDetails build() {
            final var _resultValue = new DrProtectionGroupMemberFileSystemOperationMountDetails();
            _resultValue.mountTargetId = mountTargetId;
            return _resultValue;
        }
    }
}
