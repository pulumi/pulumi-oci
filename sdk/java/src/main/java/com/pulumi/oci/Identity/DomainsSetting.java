 ...
 *             .csrAccess(var_.setting_csr_access())
 *             .idcsEndpoint(data.oci_identity_domain().test_domain().url())
 *             .schemas(&#34;urn:ietf:params:scim:schemas:oracle:idcs:Settings&#34;)
 *             .settingId(&#34;Settings&#34;)
 *             .accountAlwaysTrustScope(var_.setting_account_always_trust_scope())
 *             .allowedDomains(&#34;test.com&#34;)
 *             .allowedForgotPasswordFlowReturnUrls(var_.setting_allowed_forgot_password_flow_return_urls())
 *             .allowedNotificationRedirectUrls(var_.setting_allowed_notification_redirect_urls())
 *             .attributeSets(&#34;all&#34;)
 *             .attributes(&#34;&#34;)
 *             .auditEventRetentionPeriod(var_.setting_audit_event_retention_period())
 *             .authorization(var_.setting_authorization())
 *             .certificateValidation(DomainsSettingCertificateValidationArgs.builder()
 *                 .crlCheckOnOcspFailureEnabled(var_.setting_certificate_validation_crl_check_on_ocsp_failure_enabled())
 *                 .crlEnabled(var_.setting_certificate_validation_crl_enabled())
 *                 .crlLocation(var_.setting_certificate_validation_crl_location())
 *                 .crlRefreshInterval(var_.setting_certificate_validation_crl_refresh_interval())
 *                 .ocspEnabled(var_.setting_certificate_validation_ocsp_enabled())
 *                 .ocspResponderUrl(var_.setting_certificate_validation_ocsp_responder_url())
 *                 .ocspSettingsResponderUrlPreferred(var_.setting_certificate_validation_ocsp_settings_responder_url_preferred())
 *                 .ocspSigningCertificateAlias(var_.setting_certificate_validation_ocsp_signing_certificate_alias())
 *                 .ocspTimeoutDuration(var_.setting_certificate_validation_ocsp_timeout_duration())
 *                 .ocspUnknownResponseStatusAllowed(var_.setting_certificate_validation_ocsp_unknown_response_status_allowed())
 *                 .build())
 *             .cloudGateCorsSettings(DomainsSettingCloudGateCorsSettingsArgs.builder()
 *                 .cloudGateCorsAllowNullOrigin(var_.setting_cloud_gate_cors_settings_cloud_gate_cors_allow_null_origin())
 *                 .cloudGateCorsAllowedOrigins(&#34;https://test.com&#34;)
 *                 .cloudGateCorsEnabled(var_.setting_cloud_gate_cors_settings_cloud_gate_cors_enabled())
 *                 .cloudGateCorsExposedHeaders(var_.setting_cloud_gate_cors_settings_cloud_gate_cors_exposed_headers())
 *                 .cloudGateCorsMaxAge(var_.setting_cloud_gate_cors_settings_cloud_gate_cors_max_age())
 *                 .build())
 *             .cloudMigrationCustomUrl(var_.setting_cloud_migration_custom_url())
 *             .cloudMigrationUrlEnabled(var_.setting_cloud_migration_url_enabled())
 *             .companyNames(DomainsSettingCompanyNameArgs.builder()
 *                 .locale(var_.setting_company_names_locale())
 *                 .value(var_.setting_company_names_value())
 *                 .build())
 *             .contactEmails(&#34;contactEmails@test.com&#34;)
 *             .customBranding(var_.setting_custom_branding())
 *             .customCssLocation(var_.setting_custom_css_location())
 *             .customHtmlLocation(var_.setting_custom_html_location())
 *             .customTranslation(var_.setting_custom_translation())
 *             .defaultTrustScope(var_.setting_default_trust_scope())
 *             .diagnosticLevel(var_.setting_diagnostic_level())
 *             .diagnosticRecordForSearchIdentifiesReturnedResources(var_.setting_diagnostic_record_for_search_identifies_returned_resources())
 *             .enableTermsOfUse(var_.setting_enable_terms_of_use())
 *             .externalId(&#34;externalId&#34;)
 *             .iamUpstSessionExpiry(var_.setting_iam_upst_session_expiry())
 *             .id(var_.setting_id())
 *             .images(DomainsSettingImageArgs.builder()
 *                 .type(var_.setting_images_type())
 *                 .value(var_.setting_images_value())
 *                 .display(var_.setting_images_display())
 *                 .build())
 *             .isHostedPage(var_.setting_is_hosted_page())
 *             .issuer(var_.setting_issuer())
 *             .locale(var_.setting_locale())
 *             .loginTexts(DomainsSettingLoginTextArgs.builder()
 *                 .locale(var_.setting_login_texts_locale())
 *                 .value(var_.setting_login_texts_value())
 *                 .build())
 *             .maxNoOfAppCmvaToReturn(var_.setting_max_no_of_app_cmva_to_return())
 *             .maxNoOfAppRoleMembersToReturn(var_.setting_max_no_of_app_role_members_to_return())
 *             .ocid(var_.setting_ocid())
 *             .preferredLanguage(var_.setting_preferred_language())
 *             .prevIssuer(var_.setting_prev_issuer())
 *             .privacyPolicyUrl(var_.setting_privacy_policy_url())
 *             .purgeConfigs(DomainsSettingPurgeConfigArgs.builder()
 *                 .resourceName(&#34;resourceName&#34;)
 *                 .retentionPeriod(var_.setting_purge_configs_retention_period())
 *                 .build())
 *             .reAuthFactors(&#34;password&#34;)
 *             .reAuthWhenChangingMyAuthenticationFactors(var_.setting_re_auth_when_changing_my_authentication_factors())
 *             .resourceTypeSchemaVersion(var_.setting_resource_type_schema_version())
 *             .serviceAdminCannotListOtherUsers(var_.setting_service_admin_cannot_list_other_users())
 *             .signingCertPublicAccess(var_.setting_signing_cert_public_access())
 *             .subMappingAttr(var_.setting_sub_mapping_attr())
 *             .tags(DomainsSettingTagArgs.builder()
 *                 .key(var_.setting_tags_key())
 *                 .value(var_.setting_tags_value())
 *                 .build())
 *             .tenantCustomClaims(DomainsSettingTenantCustomClaimArgs.builder()
 *                 .allScopes(var_.setting_tenant_custom_claims_all_scopes())
 *                 .expression(var_.setting_tenant_custom_claims_expression())
 *                 .mode(var_.setting_tenant_custom_claims_mode())
 *                 .name(var_.setting_tenant_custom_claims_name())
 *                 .tokenType(var_.setting_tenant_custom_claims_token_type())
 *                 .value(var_.setting_tenant_custom_claims_value())
 *                 .scopes(&#34;scopes&#34;)
 *                 .build())
 *             .termsOfUseUrl(var_.setting_terms_of_use_url())
 *             .timezone(var_.setting_timezone())
 *             .build());
 * 
 *     }
 * }
...
public class DomainsSetting extends com.pulumi.resources.CustomResource {
    ...
    public DomainsSetting(String name, DomainsSettingArgs args, @Nullable com.pulumi.resources.CustomResourceOptions options) {
        super("oci:Identity/domainsSetting:DomainsSetting", name, args == null ? DomainsSettingArgs.Empty : args, makeResourceOptions(options, Codegen.empty()));
    }
