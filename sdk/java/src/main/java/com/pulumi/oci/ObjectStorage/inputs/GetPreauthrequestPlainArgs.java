// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.oci.ObjectStorage.inputs;

import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;


public final class GetPreauthrequestPlainArgs extends com.pulumi.resources.InvokeArgs {

    public static final GetPreauthrequestPlainArgs Empty = new GetPreauthrequestPlainArgs();

    /**
     * The name of the bucket. Avoid entering confidential information. Example: `my-new-bucket1`
     * 
     */
    @Import(name="bucket", required=true)
    private String bucket;

    /**
     * @return The name of the bucket. Avoid entering confidential information. Example: `my-new-bucket1`
     * 
     */
    public String bucket() {
        return this.bucket;
    }

    /**
     * The Object Storage namespace used for the request.
     * 
     */
    @Import(name="namespace", required=true)
    private String namespace;

    /**
     * @return The Object Storage namespace used for the request.
     * 
     */
    public String namespace() {
        return this.namespace;
    }

    /**
     * The unique identifier for the pre-authenticated request. This can be used to manage operations against the pre-authenticated request, such as GET or DELETE.
     * 
     */
    @Import(name="parId", required=true)
    private String parId;

    /**
     * @return The unique identifier for the pre-authenticated request. This can be used to manage operations against the pre-authenticated request, such as GET or DELETE.
     * 
     */
    public String parId() {
        return this.parId;
    }

    private GetPreauthrequestPlainArgs() {}

    private GetPreauthrequestPlainArgs(GetPreauthrequestPlainArgs $) {
        this.bucket = $.bucket;
        this.namespace = $.namespace;
        this.parId = $.parId;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetPreauthrequestPlainArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetPreauthrequestPlainArgs $;

        public Builder() {
            $ = new GetPreauthrequestPlainArgs();
        }

        public Builder(GetPreauthrequestPlainArgs defaults) {
            $ = new GetPreauthrequestPlainArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param bucket The name of the bucket. Avoid entering confidential information. Example: `my-new-bucket1`
         * 
         * @return builder
         * 
         */
        public Builder bucket(String bucket) {
            $.bucket = bucket;
            return this;
        }

        /**
         * @param namespace The Object Storage namespace used for the request.
         * 
         * @return builder
         * 
         */
        public Builder namespace(String namespace) {
            $.namespace = namespace;
            return this;
        }

        /**
         * @param parId The unique identifier for the pre-authenticated request. This can be used to manage operations against the pre-authenticated request, such as GET or DELETE.
         * 
         * @return builder
         * 
         */
        public Builder parId(String parId) {
            $.parId = parId;
            return this;
        }

        public GetPreauthrequestPlainArgs build() {
            if ($.bucket == null) {
                throw new MissingRequiredPropertyException("GetPreauthrequestPlainArgs", "bucket");
            }
            if ($.namespace == null) {
                throw new MissingRequiredPropertyException("GetPreauthrequestPlainArgs", "namespace");
            }
            if ($.parId == null) {
                throw new MissingRequiredPropertyException("GetPreauthrequestPlainArgs", "parId");
            }
            return $;
        }
    }

}
