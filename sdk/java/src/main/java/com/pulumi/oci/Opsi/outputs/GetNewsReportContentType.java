// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.oci.Opsi.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.List;
import java.util.Objects;

@CustomType
public final class GetNewsReportContentType {
    /**
     * @return Supported resources for actionable insights content type.
     * 
     */
    private List<String> actionableInsightsResources;
    /**
     * @return Supported resources for capacity planning content type.
     * 
     */
    private List<String> capacityPlanningResources;
    /**
     * @return Supported resources for SQL insights - fleet analysis content type.
     * 
     */
    private List<String> sqlInsightsFleetAnalysisResources;
    /**
     * @return Supported resources for SQL insights - performance degradation content type.
     * 
     */
    private List<String> sqlInsightsPerformanceDegradationResources;
    /**
     * @return Supported resources for SQL insights - plan changes content type.
     * 
     */
    private List<String> sqlInsightsPlanChangesResources;
    /**
     * @return Supported resources for SQL insights - top databases content type.
     * 
     */
    private List<String> sqlInsightsTopDatabasesResources;
    /**
     * @return Supported resources for SQL insights - top SQL by insights content type.
     * 
     */
    private List<String> sqlInsightsTopSqlByInsightsResources;
    /**
     * @return Supported resources for SQL insights - top SQL content type.
     * 
     */
    private List<String> sqlInsightsTopSqlResources;

    private GetNewsReportContentType() {}
    /**
     * @return Supported resources for actionable insights content type.
     * 
     */
    public List<String> actionableInsightsResources() {
        return this.actionableInsightsResources;
    }
    /**
     * @return Supported resources for capacity planning content type.
     * 
     */
    public List<String> capacityPlanningResources() {
        return this.capacityPlanningResources;
    }
    /**
     * @return Supported resources for SQL insights - fleet analysis content type.
     * 
     */
    public List<String> sqlInsightsFleetAnalysisResources() {
        return this.sqlInsightsFleetAnalysisResources;
    }
    /**
     * @return Supported resources for SQL insights - performance degradation content type.
     * 
     */
    public List<String> sqlInsightsPerformanceDegradationResources() {
        return this.sqlInsightsPerformanceDegradationResources;
    }
    /**
     * @return Supported resources for SQL insights - plan changes content type.
     * 
     */
    public List<String> sqlInsightsPlanChangesResources() {
        return this.sqlInsightsPlanChangesResources;
    }
    /**
     * @return Supported resources for SQL insights - top databases content type.
     * 
     */
    public List<String> sqlInsightsTopDatabasesResources() {
        return this.sqlInsightsTopDatabasesResources;
    }
    /**
     * @return Supported resources for SQL insights - top SQL by insights content type.
     * 
     */
    public List<String> sqlInsightsTopSqlByInsightsResources() {
        return this.sqlInsightsTopSqlByInsightsResources;
    }
    /**
     * @return Supported resources for SQL insights - top SQL content type.
     * 
     */
    public List<String> sqlInsightsTopSqlResources() {
        return this.sqlInsightsTopSqlResources;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GetNewsReportContentType defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private List<String> actionableInsightsResources;
        private List<String> capacityPlanningResources;
        private List<String> sqlInsightsFleetAnalysisResources;
        private List<String> sqlInsightsPerformanceDegradationResources;
        private List<String> sqlInsightsPlanChangesResources;
        private List<String> sqlInsightsTopDatabasesResources;
        private List<String> sqlInsightsTopSqlByInsightsResources;
        private List<String> sqlInsightsTopSqlResources;
        public Builder() {}
        public Builder(GetNewsReportContentType defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.actionableInsightsResources = defaults.actionableInsightsResources;
    	      this.capacityPlanningResources = defaults.capacityPlanningResources;
    	      this.sqlInsightsFleetAnalysisResources = defaults.sqlInsightsFleetAnalysisResources;
    	      this.sqlInsightsPerformanceDegradationResources = defaults.sqlInsightsPerformanceDegradationResources;
    	      this.sqlInsightsPlanChangesResources = defaults.sqlInsightsPlanChangesResources;
    	      this.sqlInsightsTopDatabasesResources = defaults.sqlInsightsTopDatabasesResources;
    	      this.sqlInsightsTopSqlByInsightsResources = defaults.sqlInsightsTopSqlByInsightsResources;
    	      this.sqlInsightsTopSqlResources = defaults.sqlInsightsTopSqlResources;
        }

        @CustomType.Setter
        public Builder actionableInsightsResources(List<String> actionableInsightsResources) {
            if (actionableInsightsResources == null) {
              throw new MissingRequiredPropertyException("GetNewsReportContentType", "actionableInsightsResources");
            }
            this.actionableInsightsResources = actionableInsightsResources;
            return this;
        }
        public Builder actionableInsightsResources(String... actionableInsightsResources) {
            return actionableInsightsResources(List.of(actionableInsightsResources));
        }
        @CustomType.Setter
        public Builder capacityPlanningResources(List<String> capacityPlanningResources) {
            if (capacityPlanningResources == null) {
              throw new MissingRequiredPropertyException("GetNewsReportContentType", "capacityPlanningResources");
            }
            this.capacityPlanningResources = capacityPlanningResources;
            return this;
        }
        public Builder capacityPlanningResources(String... capacityPlanningResources) {
            return capacityPlanningResources(List.of(capacityPlanningResources));
        }
        @CustomType.Setter
        public Builder sqlInsightsFleetAnalysisResources(List<String> sqlInsightsFleetAnalysisResources) {
            if (sqlInsightsFleetAnalysisResources == null) {
              throw new MissingRequiredPropertyException("GetNewsReportContentType", "sqlInsightsFleetAnalysisResources");
            }
            this.sqlInsightsFleetAnalysisResources = sqlInsightsFleetAnalysisResources;
            return this;
        }
        public Builder sqlInsightsFleetAnalysisResources(String... sqlInsightsFleetAnalysisResources) {
            return sqlInsightsFleetAnalysisResources(List.of(sqlInsightsFleetAnalysisResources));
        }
        @CustomType.Setter
        public Builder sqlInsightsPerformanceDegradationResources(List<String> sqlInsightsPerformanceDegradationResources) {
            if (sqlInsightsPerformanceDegradationResources == null) {
              throw new MissingRequiredPropertyException("GetNewsReportContentType", "sqlInsightsPerformanceDegradationResources");
            }
            this.sqlInsightsPerformanceDegradationResources = sqlInsightsPerformanceDegradationResources;
            return this;
        }
        public Builder sqlInsightsPerformanceDegradationResources(String... sqlInsightsPerformanceDegradationResources) {
            return sqlInsightsPerformanceDegradationResources(List.of(sqlInsightsPerformanceDegradationResources));
        }
        @CustomType.Setter
        public Builder sqlInsightsPlanChangesResources(List<String> sqlInsightsPlanChangesResources) {
            if (sqlInsightsPlanChangesResources == null) {
              throw new MissingRequiredPropertyException("GetNewsReportContentType", "sqlInsightsPlanChangesResources");
            }
            this.sqlInsightsPlanChangesResources = sqlInsightsPlanChangesResources;
            return this;
        }
        public Builder sqlInsightsPlanChangesResources(String... sqlInsightsPlanChangesResources) {
            return sqlInsightsPlanChangesResources(List.of(sqlInsightsPlanChangesResources));
        }
        @CustomType.Setter
        public Builder sqlInsightsTopDatabasesResources(List<String> sqlInsightsTopDatabasesResources) {
            if (sqlInsightsTopDatabasesResources == null) {
              throw new MissingRequiredPropertyException("GetNewsReportContentType", "sqlInsightsTopDatabasesResources");
            }
            this.sqlInsightsTopDatabasesResources = sqlInsightsTopDatabasesResources;
            return this;
        }
        public Builder sqlInsightsTopDatabasesResources(String... sqlInsightsTopDatabasesResources) {
            return sqlInsightsTopDatabasesResources(List.of(sqlInsightsTopDatabasesResources));
        }
        @CustomType.Setter
        public Builder sqlInsightsTopSqlByInsightsResources(List<String> sqlInsightsTopSqlByInsightsResources) {
            if (sqlInsightsTopSqlByInsightsResources == null) {
              throw new MissingRequiredPropertyException("GetNewsReportContentType", "sqlInsightsTopSqlByInsightsResources");
            }
            this.sqlInsightsTopSqlByInsightsResources = sqlInsightsTopSqlByInsightsResources;
            return this;
        }
        public Builder sqlInsightsTopSqlByInsightsResources(String... sqlInsightsTopSqlByInsightsResources) {
            return sqlInsightsTopSqlByInsightsResources(List.of(sqlInsightsTopSqlByInsightsResources));
        }
        @CustomType.Setter
        public Builder sqlInsightsTopSqlResources(List<String> sqlInsightsTopSqlResources) {
            if (sqlInsightsTopSqlResources == null) {
              throw new MissingRequiredPropertyException("GetNewsReportContentType", "sqlInsightsTopSqlResources");
            }
            this.sqlInsightsTopSqlResources = sqlInsightsTopSqlResources;
            return this;
        }
        public Builder sqlInsightsTopSqlResources(String... sqlInsightsTopSqlResources) {
            return sqlInsightsTopSqlResources(List.of(sqlInsightsTopSqlResources));
        }
        public GetNewsReportContentType build() {
            final var _resultValue = new GetNewsReportContentType();
            _resultValue.actionableInsightsResources = actionableInsightsResources;
            _resultValue.capacityPlanningResources = capacityPlanningResources;
            _resultValue.sqlInsightsFleetAnalysisResources = sqlInsightsFleetAnalysisResources;
            _resultValue.sqlInsightsPerformanceDegradationResources = sqlInsightsPerformanceDegradationResources;
            _resultValue.sqlInsightsPlanChangesResources = sqlInsightsPlanChangesResources;
            _resultValue.sqlInsightsTopDatabasesResources = sqlInsightsTopDatabasesResources;
            _resultValue.sqlInsightsTopSqlByInsightsResources = sqlInsightsTopSqlByInsightsResources;
            _resultValue.sqlInsightsTopSqlResources = sqlInsightsTopSqlResources;
            return _resultValue;
        }
    }
}
