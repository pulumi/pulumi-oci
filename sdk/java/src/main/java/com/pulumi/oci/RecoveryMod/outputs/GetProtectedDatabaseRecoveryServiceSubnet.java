// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.oci.RecoveryMod.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;

@CustomType
public final class GetProtectedDatabaseRecoveryServiceSubnet {
    /**
     * @return Recovery Service Subnet Identifier.
     * 
     */
    private String recoveryServiceSubnetId;
    /**
     * @return The current state of the Protected Database.
     * 
     */
    private String state;

    private GetProtectedDatabaseRecoveryServiceSubnet() {}
    /**
     * @return Recovery Service Subnet Identifier.
     * 
     */
    public String recoveryServiceSubnetId() {
        return this.recoveryServiceSubnetId;
    }
    /**
     * @return The current state of the Protected Database.
     * 
     */
    public String state() {
        return this.state;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GetProtectedDatabaseRecoveryServiceSubnet defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private String recoveryServiceSubnetId;
        private String state;
        public Builder() {}
        public Builder(GetProtectedDatabaseRecoveryServiceSubnet defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.recoveryServiceSubnetId = defaults.recoveryServiceSubnetId;
    	      this.state = defaults.state;
        }

        @CustomType.Setter
        public Builder recoveryServiceSubnetId(String recoveryServiceSubnetId) {
            if (recoveryServiceSubnetId == null) {
              throw new MissingRequiredPropertyException("GetProtectedDatabaseRecoveryServiceSubnet", "recoveryServiceSubnetId");
            }
            this.recoveryServiceSubnetId = recoveryServiceSubnetId;
            return this;
        }
        @CustomType.Setter
        public Builder state(String state) {
            if (state == null) {
              throw new MissingRequiredPropertyException("GetProtectedDatabaseRecoveryServiceSubnet", "state");
            }
            this.state = state;
            return this;
        }
        public GetProtectedDatabaseRecoveryServiceSubnet build() {
            final var _resultValue = new GetProtectedDatabaseRecoveryServiceSubnet();
            _resultValue.recoveryServiceSubnetId = recoveryServiceSubnetId;
            _resultValue.state = state;
            return _resultValue;
        }
    }
}
