...
class GetRepositoryPathsResult:
    ...
    def paths_in_subtree(self) -> Optional[_builtins.bool]:
...
def get_repository_paths(display_name: Optional[_builtins.str] = None,
    ...
    :param _builtins.str folder_path: The fully qualified path to the folder whose contents are returned, including the folder name. For example, /examples is a fully-qualified path to a folder named examples that was created off of the root directory (/) of a repository.
    :param _builtins.bool paths_in_subtree: Flag to determine if files must be retrived recursively. Flag is False by default.
    :param _builtins.str ref: The name of branch/tag or commit hash it points to. If names conflict, order of preference is commit > branch > tag. You can disambiguate with "heads/foobar" and "tags/foobar". If left blank repository's default branch will be used.
...
def get_repository_paths_output(display_name: Optional[pulumi.Input[Optional[_builtins.str]]] = None,
