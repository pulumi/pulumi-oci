...
class GetDomainsApiKeysResult:
    ...
    def api_key_count(self) -> Optional[int]:
        return pulumi.get(self, "api_key_count")

    @property
    @pulumi.getter(name="apiKeyFilter")
    def api_key_filter(self) -> Optional[str]:
...
def get_domains_api_keys(api_key_count: Optional[int] = None,
    ...
    :param int api_key_count: OPTIONAL. An integer that indicates the desired maximum number of query results per page. 1000 is the largest value that you can use. See the Pagination section of the System for Cross-Domain Identity Management Protocol specification for more information. (Section 3.4.2.4).
    :param str api_key_filter: OPTIONAL. The filter string that is used to request a subset of resources. The filter string MUST be a valid filter expression. See the Filtering section of the SCIM specification for more information (Section 3.4.2.2). The string should contain at least one condition that each item must match in order to be returned in the search results. Each condition specifies an attribute, an operator, and a value. Conditions within a filter can be connected by logical operators (such as AND and OR). Sets of conditions can be grouped together using parentheses.
...
def get_domains_api_keys_output(api_key_count: Optional[pulumi.Input[Optional[int]]] = None,
                                api_key_filter: Optional[pulumi.Input[Optional[str]]] = None,
