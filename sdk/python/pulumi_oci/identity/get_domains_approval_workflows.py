...
class GetDomainsApprovalWorkflowsResult:
    ...
    def approval_workflow_count(self) -> Optional[_builtins.int]:
        return pulumi.get(self, "approval_workflow_count")

    @_builtins.property
    @pulumi.getter(name="approvalWorkflowFilter")
    def approval_workflow_filter(self) -> Optional[_builtins.str]:
...
def get_domains_approval_workflows(approval_workflow_count: Optional[_builtins.int] = None,
                                   approval_workflow_filter: Optional[_builtins.str] = None,
    ...
    :param _builtins.int approval_workflow_count: OPTIONAL. An integer that indicates the desired maximum number of query results per page. 1000 is the largest value that you can use. See the Pagination section of the System for Cross-Domain Identity Management Protocol specification for more information. (Section 3.4.2.4).
    :param _builtins.str approval_workflow_filter: OPTIONAL. The filter string that is used to request a subset of resources. The filter string MUST be a valid filter expression. See the Filtering section of the SCIM specification for more information (Section 3.4.2.2). The string should contain at least one condition that each item must match in order to be returned in the search results. Each condition specifies an attribute, an operator, and a value. Conditions within a filter can be connected by logical operators (such as AND and OR). Sets of conditions can be grouped together using parentheses.
